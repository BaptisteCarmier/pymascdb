...
def process_basic_descriptors(
    ...
    if 'image' in regionprops_roi and regionprops_roi['image'] is not None:
        ...
        if 'coords' in regionprops_roi and regionprops_roi['coords'] is not None:
            ...
            valid = ((local_coords[:, 0] >= 0) & (local_coords[:, 0] < roi_data.shape[0]) &
                     (local_coords[:, 1] >= 0) & (local_coords[:, 1] < roi_data.shape[1]))
    ...
    if len(pixels_filled) > 0:
        ...
        roi['contrast'] = float(roi['max_intens'] - roi['min_intens']) / roi['mean_intens'] # Note : done later un matlab but coherent here
    ...
    # Conversion: matlab_deg = 90 - degrees(skimage_rad)
    ...
    orientation_deg = 90.0 - np.degrees(orientation_rad)  # Convert skimage→MATLAB convention
    ...
    if local_props:
        ...
        roi['centroid_local'] = (float(_lc[1]), float(_lc[0]))  # (x=col, y=row) like MATLAB
    else:
