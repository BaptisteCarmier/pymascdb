...
def fmeasure(image: np.ndarray, measure: str, roi: Optional[Tuple[int, int, int, int]] = None) -> float:
    ...
    if measure == "CURV":
        ...
        M1 = np.array([[-1, 0, 1], [-1, 0, 1], [-1, 0, 1]], dtype=float)

        M2 = np.array([[1, 0, 1], [1, 0, 1], [1, 0, 1]], dtype=float)

        P0 = ndimage.convolve(image.astype(float), M1, mode="nearest") / 6
        P1 = ndimage.convolve(image.astype(float), M1.T, mode="nearest") / 6
        P2 = (
            3 * ndimage.convolve(image.astype(float), M2, mode="nearest") / 10
            - ndimage.convolve(image.astype(float), M2.T, mode="nearest") / 5
        )
        P3 = (
            -ndimage.convolve(image.astype(float), M2, mode="nearest") / 5
            + 3 * ndimage.convolve(image.astype(float), M2.T, mode="nearest") / 10
        )  ### P3 peut être faux, avec la trasposition??
    ...
    if measure == "LAPD":
        ...
        M2 = np.array([[0, 0, -1], [0, 2, 0], [-1, 0, 0]], dtype=float) / np.sqrt(2)

        M3 = np.array([[-1, 0, 0], [0, 2, 0], [0, 0, -1]], dtype=float) / np.sqrt(2)
    ...
    if measure == "SFIL":
        ...
        R[:, :, 2] = (
            np.cos(np.radians(45)) * R[:, :, 0] + np.sin(np.radians(45)) * R[:, :, 1]
        )  ## np.cos() and np.sin() take radians as input
    ...
    if measure == "WAVS":
        ...
        coeffs = pywt.dwt2(
            image.astype(float), "db6"
        )  # dwt2 have no level parameter, it does one level by default, different from wavedec2 in matlab where level can be specified
        cA, (cH, cV, cD) = coeffs
    ...
    if measure == "WAVR":
        ...
        coeffs = pywt.wavedec2(
            image.astype(float), "db6", level=3
        )  # use of wavedec2 to get multiple levels, as level 3 is mentionned in matlab original code
