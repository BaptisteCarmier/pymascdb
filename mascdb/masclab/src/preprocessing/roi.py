    ...
    def _filter_border_rois(
        ...
        for roi in rois:
            ...
            if cam_id == 1:  # Left LED camera
                valid = (
                    dist_from_top > self.config.discardmat[0] + 1
                    and dist_from_bot > self.config.discardmat[1] + 1
                    and dist_from_left > self.config.discardmat[2] + 1
                    and dist_from_right > 1
                )
            # MATLAB: cam == process.camera_order(3) means cam == 3 (right LED)
                ...
                valid = (
                    dist_from_top > self.config.discardmat[0] + 1
                    and dist_from_bot > self.config.discardmat[1] + 1
                    and dist_from_right > self.config.discardmat[3] + 1
                    and dist_from_left > 1
                )
