...
def masking(flake_data: np.ndarray, flake_cam: int, process: ProcessingConfig) -> np.ndarray:
    ...
    if flake_cam == 0:  # LED on left
        data[:, :process.discardmat[2]] = 0

    elif flake_cam == 2:  # LED on right
        data[:, -process.discardmat[3]:] = 0
    
    data[:process.discardmat[0], :] = 0  # Top
    data[-process.discardmat[1]:, :] = 0  # Bottom
