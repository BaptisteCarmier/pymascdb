...
def process_triplet_image(
    ...
    for k, mid_roi in enumerate(mid_all_roi):
        ...
        matching_tol_height = max(process.matching_tol_pix, 
                                   process.matching_tol_percent * mid_height / 100.0)
        matching_tol_vert_pos = max(process.matching_tol_pix, 
                                     process.matching_tol_percent * mid_height / 100.0)
        ...
        for left_idx, left_roi in enumerate(left_all_roi):
            ...
            if (abs(left_height - mid_height) <= matching_tol_height and
                abs(left_vert_pos - mid_vert_pos) <= matching_tol_vert_pos):
        ...
        for right_idx, right_roi in enumerate(right_all_roi):
            ...
            # Check if within tolerance
            if (abs(right_height - mid_height) <= matching_tol_height and
                abs(right_vert_pos - mid_vert_pos) <= matching_tol_vert_pos):
    ...
    if process.saveresults:
        ...
        is_good = (triplet_mean_intens >= process.minbright and
                   triplet_max_intens >= process.max_intensthresh and
                   triplet_max_dim >= process.sizemin)
